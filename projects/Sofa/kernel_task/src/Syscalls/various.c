...
static void RequestCapEndoint(Thread* caller, seL4_MessageInfo_t info)
{
...
static void RequestCapTCB(Thread* caller, seL4_MessageInfo_t info)
{
    vka_object_t tcb_obj;
    vka_alloc_tcb(&getKernelTaskContext()->vka, &tcb_obj);
    cspacepath_t res;
...
static void RequestIPCBuff(Thread* caller, seL4_MessageInfo_t info)
{
    Process* process = caller->_base.process;
    assert(process);
    KernelTaskContext* env = getKernelTaskContext();
    ...
    KLOG_TRACE("BufAddr %p\n", bufAddr);
    seL4_SetMR(1, (seL4_Word) bufAddr);
    seL4_SetMR(2, ret);    
    seL4_Reply(info);
}

static void RequestNewThread2(Thread* caller,seL4_MessageInfo_t info)
{
    KernelTaskContext* ctx = getKernelTaskContext();
    Process* process = caller->_base.process;
...
static void RequestNewThread(Thread* caller, seL4_MessageInfo_t info)
{
    Process* process = caller->_base.process;
    KernelTaskContext* env = getKernelTaskContext();
    void* p = vspace_new_sized_stack(&caller->_base.process->native.vspace, 1);
...
void Syscall_RequestCap(Thread* caller, seL4_MessageInfo_t info)
{
    ...
    switch (op)
    {
    case SofaRequestCap_NewThread2:
        RequestNewThread2(caller, info);
        break;
